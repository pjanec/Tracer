...
| DT-027 | P2 | BATCH-36 | ✅ **Fixed BATCH-37.** `EntityFastStateService.GetAvailableTopics` returns safe-encoded topic names. When frontend passes them back, `LocateFiles` would double-encode. Fixed by adding `FastStateFileLocator.LocateFilesBySafeTopicName` (no re-encoding); `EntityFastStateService.GetSchemaAsync` and `ReadAsync` updated. | BATCH-37 | Resolved |
