...
| DT-026 | P2 | BATCH-35 | `FastStateFileLocator.GetAvailableTopicsForEntity` returns `BundleNaming.SafeFileName`-encoded directory names, not original topic names. BATCH-36 implementors must use `LocateFiles(topic, entityId)` for topic lookup, not this method. | BATCH-37+ | Open |
| DT-027 | P2 | BATCH-36 | `EntityFastStateService.GetAvailableTopics` returns safe-encoded topic names (e.g. `"game.tick_ab12"`). If the frontend passes these back to `GET /fast-state/{topic}`, `LocateFiles(topic, entityId)` re-encodes them → double-encoded path → file not found. **Must fix before TRC-P7-014.** Options: (a) reverse-decode in `LocateFiles` when input already looks encoded, (b) store original topic names in a sidecar metadata file alongside each Parquet file, (c) use reversible encoding in `BundleNaming.SafeFileName`. | BATCH-37+ | Open |
| DT-028 | P2 | BATCH-36 | Pre-existing: Running the full `Tracer.Tests.Unit` test suite (all test classes) causes testhost to hang indefinitely after ~4.5 CPU-seconds with memory growing to 169 MB. Individual subsets pass cleanly. Root cause unknown — likely a test that waits on a network port or async signal. Must investigate with `vstest /diag` or test categories before all tests can be run in CI. | BATCH-37+ | Open |
| DT-029 | P3 | BATCH-36 | `WebApiFixture` (test harness) does not initialize `LiveMultiIntervalReader` with a real DuckDB snapshot. Session queries against the empty reader return `SELECT NULL WHERE FALSE` with no columns, causing unhandled `InvalidOperationException` (500) instead of a well-formed 404. Tests that need "session not found → 404" from entity endpoints must use `ObserverFixture` or integration tests. | BATCH-37+ | Open |
